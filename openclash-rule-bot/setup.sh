...
# 下载配置文件
cat > youtube_config_filter.py << 'EOF'
...
async def run_youtube_unlock_test(query, provider):
    ...
    try:
        ...
        config_arg, removed_proxy_names = prepare_youtube_check_config(url, work_dir, provider, logger)
        if removed_proxy_names:
            logger.info(f"油管解锁测试已跳过 {len(removed_proxy_names)} 个 youtube-check.go 不支持的节点")
        cmd = ["go", "run", "youtube-check.go", "-c", config_arg]
...
services:
  telegram-bot:
    ...
    environment:
      ...
      - QOE_NIGHT_FAILURE_STRIKES=${QOE_NIGHT_FAILURE_STRIKES:-3}
      - QOE_NIGHT_RECOVERY_PASSES=${QOE_NIGHT_RECOVERY_PASSES:-5}
      - QOE_NIGHT_MIN_HOLD_SECONDS=${QOE_NIGHT_MIN_HOLD_SECONDS:-600}
      - QOE_DAY_SAMPLE_SECONDS=${QOE_DAY_SAMPLE_SECONDS:-2}
      - QOE_DAY_LOW_RATE_BPS=${QOE_DAY_LOW_RATE_BPS:-3145728}
      - QOE_ACTIVE_FLOW_MIN_AGE_SECONDS=${QOE_ACTIVE_FLOW_MIN_AGE_SECONDS:-10}
      - QOE_ACTIVE_FLOW_MIN_BYTES=${QOE_ACTIVE_FLOW_MIN_BYTES:-4194304}
      - QOE_LOW_RATE_DIRECT_TRIGGER_BPS=${QOE_LOW_RATE_DIRECT_TRIGGER_BPS:-524288}
      - QOE_DAY_FAILURE_STRIKES=${QOE_DAY_FAILURE_STRIKES:-3}
      - QOE_DAY_COOLDOWN_SECONDS=${QOE_DAY_COOLDOWN_SECONDS:-600}
EOF
